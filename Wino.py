...
X = df.drop(["quality"], axis=1).drop(["fixed acidity"], axis=1).to_numpy()
...
sigma = tf.get_variable(name="sigma", shape=[m * n],
                        initializer=tf.random_normal_initializer(0, 1))  # std_dev of Gaussian MFS
...
with tf.Session() as sess:
    ...
    for e in range(epochs):
        ...
        if (e + 1) % 10 == 0:
            ...
            prec_tr = precision_score(y_train, Y_train, zero_division=0)
            prec_te = precision_score(y_test, Y_test, zero_division=0)
        ...
        if (e + 1) % 200 == 0:
            ...
            disp = ConfusionMatrixDisplay(confusion_matrix=cm_test, display_labels=["Not Good", "Good"])
            disp = disp.plot()
            # plt.show()
            plt.savefig(dir_ + "/cf_test-epoch" + str(e + 1) + ".png", transparent=True)

            plt.figure(int(str(e + 1) + '2'))
            disp = ConfusionMatrixDisplay(confusion_matrix=cm_train, display_labels=["Not Good", "Good"])
            disp = disp.plot()
            # plt.show()
